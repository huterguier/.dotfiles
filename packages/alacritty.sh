...
case "$(uname -s)" in
  Darwin)
    if [ ! -d /Applications/Alacritty.app ]; then
      # the Homebrew cask is deprecated (the build fails Apple's Gatekeeper
      # check and the cask is disabled from 2026-09-01), so install the
      # official DMG from GitHub releases instead
      ALACRITTY_VERSION="$(curl -s https://api.github.com/repos/alacritty/alacritty/releases/latest | python3 -c 'import json,sys; print(json.load(sys.stdin)["tag_name"])')"
      curl -sL "https://github.com/alacritty/alacritty/releases/download/${ALACRITTY_VERSION}/Alacritty-${ALACRITTY_VERSION}.dmg" -o /tmp/alacritty.dmg
      MOUNT_DIR="$(hdiutil attach -nobrowse -readonly /tmp/alacritty.dmg | awk -F'\t' 'END {print $NF}')"
      cp -R "$MOUNT_DIR/Alacritty.app" /Applications/
      hdiutil detach "$MOUNT_DIR" >/dev/null
      rm /tmp/alacritty.dmg
      # the upstream build is unsigned, so clear the quarantine flag ourselves
      # instead of doing it by hand each time
